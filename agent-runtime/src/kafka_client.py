...
import time
# uuid 모듈은 유니크한 sseEventId 생성에 활용됩니다.
import uuid
...
from src.config import KAFKA_BOOTSTRAP_SERVERS, TOPIC_AGENT_EVENTS, KAFKA_CONSUMER_GROUP_ID, TOPIC_AGENT_RUNS


class AgentKafkaProducer:
    """
    파이썬 에이전트의 상태(STATUS), 토큰 청크(CHUNK), UI(A2UI_RENDER), 완료(DONE), 에러(ERROR) 이벤트를
    Kafka 이벤트 토픽(agent-events)으로 비동기 고속 발행(Produce)하는 전용 클래스입니다.
    """
    def __init__(self, bootstrap_servers: str = KAFKA_BOOTSTRAP_SERVERS):
        # Kafka 프로듀서 설정을 구성합니다.
        conf = {
            'bootstrap.servers': bootstrap_servers,
            'linger.ms': 0  # 지연 없이 즉각 패킷을 전송합니다.
        }
        self.producer = Producer(conf)

    def _delivery_report(self, err, msg):
        """메시지 발행 완료 시 호출되는 콜백 함수입니다."""
        if err is not None:
    ...
    def send_event(
        ...
        """
        AgentEvent Kafka 메시지를 블로킹 없이 논블로킹 비동기로 빠르게 전송합니다.
        """
        sse_event_id = f"evt-{uuid.uuid4().hex[:8]}"

        payload: Dict[str, Any] = {
            "sseEventId": sse_event_id,
            "runId": run_id,
        ...
        # 턴이 완료된 시점에만 flush()를 호출하고, 일반 토큰은 poll(0)으로 논블로킹 큐잉합니다.
        if event_type == "DONE":
            self.producer.flush()
        else:
...
class AgentKafkaConsumer:
    """
    Kafka 턴 요청 토픽(agent-runs)으로부터 클라이언트의 AgentRunRequest 메시지를
    ...
    def start_listening(self, message_handler: Callable[[Dict[str, Any]], None]) -> None:
        ...
        while True:
            try:
                ...
                run_dict = json.loads(raw_data)
                message_handler(run_dict)
