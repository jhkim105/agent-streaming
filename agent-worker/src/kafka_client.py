...
class AgentKafkaProducer:
    ...
    Kafka 이벤트 토픽(agent-events)으로 발행(Produce)하는 전용 클래스입니다.
    """
    ...
    def _delivery_report(self, err, msg):
        """
        카프카 메시지 전송 결과를 비동기 콜백받는 내부 함수입니다.
    ...
    def send_event(
        ...
        """
        클라이언트 게이트웨이가 이해할 수 있는 규격화된 AgentEvent Kafka 메시지를 전송합니다.
        - command_id: 사용자 커맨드 유니크 ID (UUID)
        - conversation_id: 비즈니스 리서치 대화 식별자
        - host_id: 타깃 코틀린 게이트웨이 서버 ID
        - event_type: 이벤트 종류 (STATUS | CHUNK | A2UI_RENDER | DONE | ERROR)
        - content: 텍스트 내용 (상태 로그 메시지 또는 LLM 생성 토큰 조각)
        - title: LLM이 생성한 스마트 이모지 요약 타이틀
        - step: 에이전트의 현재 추론 단계 이름
        """
        # 각 AgentEvent마다 고유한 eventId를 생성합니다.
        event_id = f"evt-{uuid.uuid4()}"

        # 카프카 응답 스키마 규격에 맞게 파이썬 딕셔너리를 구성합니다. (AgentEvent)
        ...
        # 지정된 카프카 이벤트 토픽(agent-events)으로 메시지를 전송(Produce)합니다.
        self.producer.produce(
            topic=TOPIC_AGENT_EVENTS,
            key=command_id.encode('utf-8'),  # commandId를 파티션 키로 지정
            value=value_bytes,
...
class AgentKafkaConsumer:
    ...
    def start_listening(self, message_handler: Callable[[Dict[str, Any]], None]) -> None:
        """
        무한 루프를 돌며 카프카 메시지를 폴링하고, 수신된 AgentCommand 메시지를 콜백 함수로 전달합니다.
        """
        print(f"[KafkaConsumer] Listening on topic '{TOPIC_AGENT_COMMANDS}'...")
        try:
            while True:
                ...
                try:
                    ...
                    # 수신된 AgentCommand 파이썬 딕셔너리 객체를 처리 콜백 함수로 전달합니다.
                    message_handler(command_dict)
                except Exception as e:
                    print(f"[KafkaConsumer ERROR] 커맨드 메시지 파싱 중 오류 발생: {e}")
