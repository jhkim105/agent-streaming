...
```mermaid
flowchart TD
    Client["Client Browser<br/>(Vite + React SPA)"]

    subgraph Cluster ["Kotlin Agent Stream Server Cluster"]
        Node1["Stream Server Node 1<br/>(Host ID: kotlin-node-1)<br/>- SSE Session Map (Local)<br/>- Kafka Producer/Consumer"]
        Node2["Stream Server Node 2<br/>(Host ID: kotlin-node-2)<br/>- SSE Session Map (Local)<br/>- Kafka Producer/Consumer"]
    end

    KafkaReq[("Kafka Broker<br/>Topic: agent-requests")]
    KafkaRes[("Kafka Broker<br/>Topic: agent-responses")]

    Worker["Python Agent Worker<br/>(LangGraph Multi-Step Engine)<br/>Query Analysis → DuckDuckGo Search → Web Scraper → Summarize/Stream"]

    Redis[("Redis Pub/Sub<br/>Channel: host:kotlin-node-1")]

    %% Interactions
    Client -->|"(1) GET /api/chat/stream<br/>SSE 커넥션 수립 & Session ID 생성"| Node1
    Client -->|"(2) POST /api/chat/message<br/>질문 등록 (Session ID 포함)"| Node1

    Node1 -->|"(3) Produce Request"| KafkaReq
    KafkaReq -->|"(4) Consume Request"| Worker

    Worker -->|"(5) Produce Response"| KafkaRes
    KafkaRes -->|"(6) Consume Response"| Node2

    Node2 -->|"(7) Host ID 불일치 시 Redis Pub/Sub 라우팅"| Redis
    Redis -->|"(8) Publish Message"| Node1
    Node1 -->|"(9) SSE Stream Response"| Client
```
